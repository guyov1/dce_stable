...
[CVI, BinCVI, Bin2CVI]=ChooseRepVoxelsForAIFFind(Msk3D,MskE,CTC2D(:,1:NumVols),BolusStart,Options.Rep_MaxAroundBolus,Options.Rep_RatioToEnd,Options.Rep_nPerSet);
...
%%
MskCTC=Idx3D>0;
MskCTC2=MskCTC & ApproximatePeakTime<median(ApproximatePeakTime1D)+Philips*2;
F1=find(MskCTC);
F2=find(MskCTC2);
F3=find(ismember(F1,F2));
CTC2D2=CTC2D(F3,:);
[CVI2, BinCVI2, Bin2CVI2]=ChooseRepVoxelsForAIFFind(MskCTC2,MskE,CTC2D2(:,1:NumVols),BolusStart,Options.Rep_MaxAroundBolus,Options.Rep_RatioToEnd,Options.Rep_nPerSet);

DataToFit2=CTC2D2(CVI2,:);
DataNoise=rmadCTC2D(CVI2);
if(isempty(CVI2))
    CVI2=CVI;
    BinCVI2=BinCVI;
    Bin2CVI2=Bin2CVI;
...
save(PKMFN,'OutAIFParam','DataToFit2','Options','TimeBetweenDCEVolsFinal','nSVols','GoodTs','SampleTs');