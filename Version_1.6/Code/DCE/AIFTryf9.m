...
CostFunc6=@(x) -0*min(x(1),UB(3))/8 -0*abs(min(UB(5),max(LB(5),x(3)))-0.3)/20 +AIFCostFuncMf(WorkingP,RestrainX([OldParams(1:2)' x(1:7)]),DataToFit,SampleTs,HSampleTs,TDif,WPerCTC,AIF_Parker9t);

% BaseParams=AllAIFPs(2,:);
BaseParams=OutAIFParamBeforeFMS;
% BaseParams=X0';
% X0=[T1  A1  sig1 T2Delta A2 sig2 Alpha Beta]';
x = fminsearch(CostFunc6,BaseParams(3:9),optionsB)
FMSCost=CostFunc6(x);
...
saveas(1000,[AIFFinderFN(1:end-4) '.fig']);
saveas(1000,[AIFFinderFN(1:end-4) '.png']);
close(1000);
...
% end
    ...
    saveas(100+kk,[regexprep(AIFFinderFN(1:end-4),'\.','_') '_FitOnArt_' num2str(kk) '.fig']);
    saveas(100+kk,[regexprep(AIFFinderFN(1:end-4),'\.','_') '_FitOnArt_' num2str(kk) '.png']);
    close(100+kk);
