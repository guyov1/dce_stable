...
function DCET1_CTCf(DCEInfo, Additional_T1_Maps_Time_Stamps, WorkingP,CalcForce,Options)
...
% Find kCoeff by ref
D=dir([WorkingP 'Ref_*.nii']);
if(isempty(D))
    D=dir([WorkingP 'Ref*.nii']);
end
...
set(gca,'XTick',[],'YTick',[]);
gsubplot(1,3,1,3);
imagesc(mritransform(M03D(GoodRows,GoodCols,MidSli)./DCEM0(GoodRows,GoodCols,MidSli)),[0.9 1.1])
set(gca,'XTick',[],'YTick',[]);
title('Ratio');
%%
...
figure(9384);clf;
gsubplot(4,1);
imagesc(mritransform(FinalT1(GoodRows,GoodCols,MidSli)),[0 3000]);
set(gca,'XTick',[],'YTick',[]);
colorbar;
title('Final T_1');
gsubplot(4,2);
...
gsubplot(4,3);Tmp=FinalT1-T1FromBaseline;
imagesc(mritransform(Tmp(GoodRows,GoodCols,MidSli)),[-100 100]);
set(gca,'XTick',[],'YTick',[]);
title('T_1 difference');
gsubplot(4,4);
Xs=-200:200;
bar(Xs,histc(Tmp(CT1>0),Xs));
title(['Mode: ' num2str(mode(round(Tmp(CT1>0)))) ' Med abs: ' num2str(median(abs(round(Tmp(CT1>0 & isfinite(Tmp)))))) ]);
%%
...
for ii=1:numel(BadTimePoints)
    ...
    if(all(~ismember([CurPoint-2 CurPoint-1 CurPoint+1 CurPoint+2],BadTimePoints)))
        CTC2DBig(:,CurPoint)=mean(CTC2DBig(:,[CurPoint-2 CurPoint-1 CurPoint+1 CurPoint+2]),2);
    else
        if(all(~ismember([CurPoint-1 CurPoint+1],BadTimePoints)))
            CTC2DBig(:,CurPoint)=mean(CTC2DBig(:,[CurPoint-1 CurPoint+1]),2);
        else
            if(all(~ismember([CurPoint-2 CurPoint+2],BadTimePoints)))
