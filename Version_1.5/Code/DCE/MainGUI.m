...
if(isfield(handles,'BaseP'))
    ...
    elseif strcmp('FMRI-T9',getenv('ComputerName'))
        ...
        handles.BaseP=uigetdir('Z:\WM-lesions\group 1=MS\BAVLI_YEHOSHUA\Study20121213_084054\');
...
% Calculate T1 maps
DCET1_RelaxForSubjectf(RelaxInfox,handles.destFolder,CalcForce,CrgTo,handles.Options,MainInfo);
...
DCET1_CTCf(handles.ShortInfos(GoodS(DCEMainI)),Additional_T1_Maps_Time_Stamps,handles.destFolder,CalcForce,handles.Options);
...
if(exist(MatFN,'file') && ~get(handles.Force_Read_Prot_CheckBox,'Value')) %% If exists and not "force"
    ...
    if(~exist('ShortInfos','var'))
        if(~exist(CurBaseP))
            Remove_From_List_PushButton_Callback(hObject, eventdata, handles);
