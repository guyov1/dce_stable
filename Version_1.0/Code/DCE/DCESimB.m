...
%         save(SimFN,'SBATs','SPKs','Sims','NSims','NewParams','HAIF','HAIFOrig');
        ...
        [PKOut OutAIFParam]=AIFTryf(DataToFit,MaxAmp,DataNoise,ParamAIFCoeff,nSVols,a.TimeBetweenDCEVols,Options,false,SimAIFFinderFN);
...
%         figure(2000);clf;plot(HSampleTs,HAIF,'k','LineWidth',4);hold on;plot(HSampleTs,EAIF,'m');
        ...
        for i=1:nTDif
            SAIF(i,:)=interp1(HSampleTs,EAIF,SampleTs+TDif(i),[],'extrap');
        ...
        save(SimPKM3DFN,'OutAIFParam','PKs');
        %%
        EBATs=TDif(PKs(:,1));
    catch ME
        MEs{p}=ME;
