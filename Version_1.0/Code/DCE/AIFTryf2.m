function [PKOut OutAIFParam]=AIFTryf(DataToFit,MaxAmp,DataNoise,ParamAIFCoeff,nSVols,TimeBetweenDCEVols,AOptions,ShowFig,AIFFinderFN)
...
WRegBolus=3*NFac;
WSmooth1=30*NFac;
WSmooth1Bolus=10*NFac;
WSmooth2=0.000*NFac;
WSmooth2Bolus=0.000*NFac;
...
figure(1000);clf;
subplot(2,2,1);hold on;plot(HSampleTs,X0AIF,'r','LineWidth',2);plot(HSampleTs,HAIFFromMeanVessel,'b','LineWidth',2);plot(HSampleTs,BAIF,'g','LineWidth',1);plot(SampleTs,MeanVessel,'k*');
...
%     [MIdxs CXs Sims] = FindKepBATgAIF(DataToFit,SAIF,SHConvd,Keps1I);
    ...
    [PKs Sims ARMS(IterCount)] = FindPKBATgAIFMurase(DataToFit,SAIF,SampleTs,CumSumAIFdTSamp,WPerCTC);
%     CXs=PKs(:,[3 4])';
%     MIdxs=PKs(:,1);
...
CostFunc6=@(x) -0*min(x(1),UB(3))/8 -0*abs(min(UB(5),max(LB(5),x(3)))-0.3)/20 +AIFCostFuncM(RestrainX([AllAIFPs(1,1:2) x(1:6)]),DataToFit,SampleTs,HSampleTs,TDif,WPerCTC);
...
% [PKs Sims] = FindPKBATgAIFMurase(DataToFit,SAIF,SampleTs,CSAIF);
