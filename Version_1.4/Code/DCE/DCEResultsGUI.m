...
% Last Modified by GUIDE v2.5 08-Nov-2012 20:19:14
...
NormalizeCTCs=get(handles.checkNormalizeRows,'Value');
...
BATIdx=1;
CurBATs=handles.PKs(Idxs,BATIdx);
...
if(NormalizeCTCs)
    plot(handles.HSampleTs,handles.HAIF./max(handles.HAIF),'k','LineWidth',1);hold on;
    Coeffs=max(Sims,[],2);
    Tmp=repMulti(handles.CTC2D(Idxs,:),1./Coeffs);
    plot(handles.SampleTs,Tmp','*');
else
