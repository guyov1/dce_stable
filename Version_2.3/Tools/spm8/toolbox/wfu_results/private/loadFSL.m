...
% $Revision: 1.4 $
  ...
  [statsDir statsFile statsExt junk] = fileparts(overlay);  %this is a potient fsldir, redefined below when truely known
  ...
  if strcmpi(statsExt,'.gz')
    [junk statsFile statsExt junk] = fileparts(statsFile);
  ...
  %known overlay name
  statSuffix=[];
  [statDirA statNameA statExtA junk]=fileparts(overlay);
  if strcmpi(statExtA,'.gz')
    [junk statNameA statExtA1 junk2]=fileparts(statNameA);
