...
if nargin <=1  % LAUNCH GUI
  WFU_LOG.info('Entered function $Revision: 1.16 $');
...
%		end
    ...
    if xor(handles.FlipDisplay,handles.atlasVolFlip)
        handles.data.DispVolWork=flipdim(handles.data.DispVolWork,1);
    end
    if get(handles.chkFlipUD,'Value')
        handles.data.DispVolWork=flipdim(handles.data.DispVolWork,2);
    end
    guidata(handles.WFU_PickAtlas,handles);
