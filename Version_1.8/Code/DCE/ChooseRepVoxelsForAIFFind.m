function [CVI BinCVI Bin2CVI]=ChooseRepVoxelsForAIFFind(Msk3D,MskE,CTC2DOrig,BolusStart,nSets1,nSets2,nPerSet)
...
Tmp=sort(max(CTC2DOrig,[],2));
Thresh=Tmp(max(1,floor(numel(Tmp)*0.0)));
GoodIdxsS=find(Tmp>Thresh);

% Intersect the eroded mask and the mask that checked for negative values
GoodIdxs=intersect(intersect(GoodIdxsM,GoodIdxsR),GoodIdxsS);
