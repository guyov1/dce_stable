function fit = ebbsmv01_raymodified_under(xxxx,yyyy,param) ;
%
...
%
% Here is the undersmoothing
%
bandmean   = bandmean .* (n .^ (-2 ./ 15));
%
% You have to make sure that you have not done too much
% undersmoothing. Thus, what I do here is to make
% sure that the undersmoothed bandwidth is large enough
% so that at a minimum there are 3 observations in each
% bandwidth interval. I do this by making the minimum bandwidth
% just a little bit more than the maximum spacing
%
zunder    = sort(x);
qaa       = zunder(1:(n-1),:);
qbb       = zunder(2:n,:);
qdd       = qbb - qaa;
qee       = 2 .* max(qdd);
nnnn      = length(bandmean);
hmm       = bspan(zunder,zunder,6/nnnn) ;
hmmm      = interp1(zunder,hmm,xgrid,'linear');
bandmean  = max(bandmean,qee);
bandmean  = max(bandmean,hmmm);
