...
CostFunc6=@(x) -0*min(x(1),UB(3))/8 -0*abs(min(UB(5),max(LB(5),x(3)))-0.3)/20 +AIFCostFuncM(RestrainX([OldParams(1:2)' x(1:6)]),DataToFit,SampleTs,HSampleTs,TDif,WPerCTC);
...
%         plot(SampleTs,Sims(c,:),'r');
        ...
        %         title(PKs(c,7));
        %         title([RError MError]);
        title([max(HCurSim2)./max(DataToFit(c,:)) TShift(c)]) ;
