...
% Last Modified by GUIDE v2.5 09-Oct-2012 15:47:30
...
set(handles.listbox4,'String',Strs);
...
if(exist(MatFN,'file'))
    handles=pushbutton12_Callback(hObject, eventdata, handles);
end
...
handles.GoodSers=get(handles.listbox1,'Value');
guidata(hObject, handles);
saveGUIStatus(handles);
...
GoodS=get(handles.listbox1,'Value');
...
f = helpdlg('Loading...');
if(exist(MatFN,'file') && ~get(handles.checkbox9,'Value'))
    load(MatFN);
...
set(handles.listbox1,'String',Descriptions);
set(handles.listbox1,'Max',3);
set(handles.listbox1,'Min',0);
...
set(handles.listbox1,'Value',handles.GoodSers);

guidata(hObject, handles);
saveGUIStatus(handles);

% --- Executes on button press in pushbutton13.
function handles=pushbutton13_Callback(hObject, eventdata, handles)
...
% handles=pushbutton3_Callback(hObject, eventdata, handles);
% handles=pushbutton5_Callback(hObject, eventdata, handles);
% pushbutton6_Callback(hObject, eventdata, handles);
Strs=get(handles.listbox3,'String');
...
if(TD<3e-6)
    ...
    handles.BaseP=S{get(handles.listbox2,'Value')};
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
CurPs=get(handles.listbox2,'String');
if(~iscell(CurPs))
    CurPs={CurPs};
    CurPs=CurPs(cellNumel(CurPs)>0);
end
CurPs=[CurPs; handles.BaseP];
set(handles.listbox2,'String',CurPs);
handles.CurPs=CurPs;
guidata(hObject, handles);
saveGUIStatus(handles);
...
DoDebug=get(handles.checkbox7,'Value');
for i=1:numel(CurPs)
    if(DoDebug)
        handles.BaseP=CurPs{i};
        handles=pushbutton13_Callback(hObject, eventdata, handles);
    else
        try
            handles.BaseP=CurPs{i};
            handles=pushbutton13_Callback(hObject, eventdata, handles);
...
set(handles.listbox2,'String',Strs(Others));
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.listbox2,'String',cell(0));
...
if(~iscell(Strs))
    ...
    Strs=Strs(get(handles.listbox2,'Value'));
end
ErrorList=[];
MEs=cell(1);
ProbPs=cell(1);
...
set(handles.listbox2,'String',Strs);
close(f);
guidata(hObject, handles);
saveGUIStatus(handles);
...
CurV=get(handles.listbox4,'Value');
FNames=fieldnames(handles.Options);
...
FillTo=max(cellNumel(FNames))+2;
Strs=get(handles.listbox4,'String');
Strs{i}=[FNames{i} repmat(' ',[1 FillTo-numel(FNames{i})]) num2str(handles.Options.(FNames{i}))];
set(handles.listbox4,'String',Strs);
guidata(hObject, handles);
