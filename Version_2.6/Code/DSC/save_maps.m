function [full_maps_dir_string]  = save_maps(CBF,CBV,MTT,K1,K2,TTP,Mask,deconv_methods,handles)
...
% save CBV, K1, K2, Mask, TTP - which are not dependent on the SVD method:
% Also rotate back to a "nii" position

% save CBV:
CBV_in_corr=CBV_in.corr;
CBV_in_no_corr=CBV_in.no_corr;
CBV_in_corr_norm=CBV_in.corr_norm;
...
CBV_nii_corr_norm=make_nii(permute(flipdim(CBV_in_corr_norm,1),[2 1 3 4]));
save_nii(CBV_nii_corr_norm,[full_maps_dir_string,filesep,'CBV_norm']);
save([full_maps_dir_string,filesep,'CBV_norm.mat'],'CBV_nii_corr_norm');
...
if get(handles.calcTTP_checkbox,'Value')
    TTP_nii=make_nii(permute(flipdim(TTP,1),[2 1 3]));
    save_nii(TTP_nii,[full_maps_dir_string,filesep,'TTP']);
    save([full_maps_dir_string,filesep,'TTP.mat'],'TTP');
end
