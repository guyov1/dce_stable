...
for sim_iter = 1 : 2
    ...
    eval(['Sim_Struct = Simulation_Set_Params_' num2str(sim_iter) '(Sim_Struct, Verbosity);']);
    
    % Initiate simulation vars/structs etc.
    Sim_Struct = Simulation_Init_4_Article(Sim_Struct, Verbosity);
    ...
    eval(['save(''./Saved_iter_' num2str(sim_iter) '.mat'',''results'',''E_single'',''Vb_single'',''F_single'')']);
